...
function LoginPage() {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            toast.error("Wrong Detail, Try again!");
        } finally {
            setLoading(false);
        }
    };
   ...
   return (
    <div className="min-h-screen bg-[#F6F6EF]">
        <div className="flex flex-col items-center justify-center py-15 sm:py-20 px-10">
            <div className="w-full max-w-md bg-white rounded-lg">

                <div className="mb-8 text-center">
                    <div className="flex flex-col items-center gap-2">
                        <div className="flex items-center justify-center rounded-sm bg-[#FE6603] px-2 py-2 mt-8 sm:mt-10">
                        <CiLogin size={40} className="text-white" />
                    </div>
                    <h1 className="mt-2 text-xl font-bold text-black">Login to add Post</h1>
                </div>
            </div>


            <form className="space-y-7 sm:space-y-7 px-5" onSubmit={handleSubmit}>
                <div>
                    <label>
                        <span className="text-black font-medium">Email</span>
                    </label>
                    <div className="border border-[#FF8162] mt-2 py-2 sm:py-3 rounded-md relative">
                ...
                <button type="submit" disabled={loading} className="bg-[#FE6603] hover:bg-[#FF762D] font-semibold tracking-wide rounded-sm py-2 sm:py-3 cursor-pointer text-white w-full">{loading ? "Logging..." : "Log In"}</button>
            </form>
            ...
            <div className="text-center pt-5 pb-8 sm:pb-10">
                <p className="text-black/60">
                    ...
                    <Link className="text-[#FF6600] hover:text-[#FF6600]/90 px-2" href={"/"} data-discover="true">
                        Register.
                    </Link>
                </p>
            </div>
